...
训练完成后，App 只使用真实生成的 `best.pt`，不会用官方基础权重冒充训练结果。普通 Webcam 使用 OpenCV；Gemini 336L 应选择“Orbbec SDK RGB-D（实例距离）”。

这是通用 YOLO11-seg 识别：模型训练过哪些类别，就识别哪些类别，不是黄罐专用。

Orbbec RGB-D 模式会启用 640×480@30 的硬件 Depth-to-Color 对齐和帧同步。每个
YOLO 实例默认显示 `类别 置信度 Z=距离m`；距离来自实例 Mask 内 100–5000 mm
有效深度像素的中位数，并优先轻微收缩 Mask 以减少物体边缘和背景污染。有效像素
不足时显示 `Z=--`，不会使用框中心的单个深度值冒充结果。

普通 Webcam 没有深度传感器，无法提供该距离。Linux 的 `/dev/videoN` 编号也可能
随重启或插拔变化，因此 Gemini 336L 应使用 Orbbec SDK 模式，不要依赖某个 V4L2 编号。
```text
Q / Esc / Ctrl+C   退出识别
```
