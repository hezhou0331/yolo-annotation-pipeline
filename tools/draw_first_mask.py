...
erase holes or boundary mistakes.  This is intended for the first frame of each
automatically proposed segment; each instance is saved to its own binary PNG.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="用多边形和画笔为关键帧绘制目标掩膜")
    p.add_argument("--image", type=Path, required=True, help="关键帧RGB图片")
    p.add_argument("--output", type=Path, required=True, help="输出二值mask PNG")
    p.add_argument("--mask-input", type=Path, default=None, help="可选：以已有mask为起点；默认自动读取output")
    p.add_argument("--brush-size", type=int, default=18, help="初始画笔直径，像素")
    p.add_argument("--no-resume", action="store_true", help="即使output已存在也从空mask开始")
    return p.parse_args()
...
def main() -> int:
    ...
    def on_mouse(event, x, y, flags, param):
        nonlocal drawing, previous_xy, dirty
        if mode == "polygon":
    ...
    print("[ / ]：缩小/放大画笔；M：显示/隐藏覆盖；S：保存；Q/ESC：退出。")
    while True:
        canvas = image.copy()
        if show_overlay and mask.any():
            ...
            color[:, :, 1] = mask
            canvas = cv2.addWeighted(overlay, 0.72, color, 0.28, 0)
        if points:
        ...
        operation = "擦除" if erase else "添加"
        status = f"mode={mode} op={operation} points={len(points)} brush={brush_size} px={int((mask>0).sum())}"
        cv2.rectangle(canvas, (0, 0), (min(canvas.shape[1], 980), 40), (16, 24, 40), -1)
        cv2.putText(canvas, status, (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.imshow(window, canvas)
        key = cv2.waitKey(20) & 0xFF
        if key in (ord("q"), ord("Q"), 27):
            if dirty:
                print("有未保存修改；未写入磁盘。按S保存后再退出。")
